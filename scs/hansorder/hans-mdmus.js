...
const text = m.body.slice(prefix.length + cmd.length).trim();
    ...
    const validCommands = ['shazam', 'hansmusicfinder', 'whatmusic'];
    if (!validCommands.includes(cmd)) return;
    
    const quoted = m.quoted || {}; 

    if (!quoted || (quoted.mtype !== 'audioMessage' && quoted.mtype !== 'videoMessage')) {
      return m.reply('You asked about music. Please provide a quoted audio or video message for identification by hans-md ok bro.');
    }
    ...
    try {
      ...
      m.reply('Hans-md Identifying the music, please wait...');
